...
class FilterMedicinesDepend extends Component {
  state = {
  ...
  renderTagPlaceHolder = (valueChannel) => (
    ...
    >
      bạn đã chọn {valueChannel && valueChannel.length} khách hàng
    </a>
  )

  render() {
    const { medicines, key, placeholder, filterField } = this.props;
    const { loading, value, maxTagCount } = this.state;
    const tagCountType = (value && value.length) || 0;
    const keyOption = value.map(item => item.key)
    log('filterField', filterField)

    let filteredOptions = medicines && medicines.filter(o => filterField.includes(`${o.groupMedicinesId}`))
    log('medicines', medicines)
    log('filteredOptions', filteredOptions)
    filteredOptions = filteredOptions.filter(o => !keyOption.includes(`${o.id}`))
...
export default FilterMedicinesDepend;