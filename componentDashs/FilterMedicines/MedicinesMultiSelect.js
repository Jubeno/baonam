...
class Index extends Component {
  ...
  renderTagPlaceHolder = (valueChannel) => (
    ...
    >
      bạn đã chọn {valueChannel && valueChannel.length} thuốc
    </a>
  )

  render() {
    const { medicines, key, placeholder } = this.props;
    const { loading, value, maxTagCount } = this.state;
    const tagCountType = (value && value.length) || 0;
    const keyOption = value.map(item => item.key)
    const filteredOptions = medicines && medicines.filter(o => !keyOption.includes(`${o.id}`))
