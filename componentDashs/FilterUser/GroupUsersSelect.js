...
class Index extends Component {
  ...
  renderTagPlaceHolder = (valueChannel) => (
    ...
    >
      bạn đã chọn {valueChannel && valueChannel.length} nhóm tài khoản
    </a>
  )

  render() {
    const { groupUsers, key, placeholder } = this.props;
    const { loading, value, maxTagCount } = this.state;
    const tagCountType = (value && value.length) || 0;
    const keyOption = value.map(item => item.key)
    const filteredOptions = groupUsers && groupUsers.filter(o => !keyOption.includes(`${o.id}`))
