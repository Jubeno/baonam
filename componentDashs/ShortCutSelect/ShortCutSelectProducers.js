...
import GroupProducer from '@/componentDashs/FilterMedicines/GroupProducer'
...
@Form.create()
@injectIntl
class ShortCutSelecProducers extends Component {
  state = {
        ...
        dispatch({
          ...
          callback: (result) => {
            if (result && result.success === true) {
              ...
              message.success(intl.formatMessage({ id: 'app.common.create.success' }, { name: 'nhà sản xuất' }));
              setTimeout(() => {
                this.setState({
                  modalVisible: false,
                  spinning: false
    ...
    return (
      <React.Fragment>
        <Row gutter={{ md: 6 }}>
          ...
          <Col span={1}>
            <Tooltip title="Thêm nhanh nhà sản xuất">
        ...
        <Spin spinning={spinning}>
          <Modal
            destroyOnClose
            title="Thêm nhanh nhà sản xuất"
            visible={modalVisible}
            onOk={this.handleSubmit}
            onCancel={this.handleModalCancel}
          >

            <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 15 }} label="Tên nhà sản xuất">
            ...
            <FormItem labelCol={{ span: 8 }} wrapperCol={{ span: 15 }} label="Nhóm nhà sản xuất">
              {form.getFieldDecorator('groupProducersId', {
                initialValue: '',
                rules: [{
                  required: true,
                ...
                <GroupProducer
                  // style={{ width: '100%' }}
                  placeholder='Chọn nhóm nhà sản xuất'
                />
              )}
            </FormItem>
...
export default ShortCutSelecProducers;
