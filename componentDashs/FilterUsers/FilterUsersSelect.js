...
@connect(({ users: { dataAll } }) => ({
  users: dataAll || []
}))
class FilterUsersSelect extends Component {
  state = {
  ...
  renderData = data =>
    (data || []).map(item => ({ key: `${item.id}`, value: `${item.fullname}` }));
  ...
  renderTagPlaceHolder = (valueChannel) => (
    ...
    >
      bạn đã chọn {valueChannel && valueChannel.length} nhân viên
    </a>
  )

  render() {
    const { users, key, placeholder } = this.props;
    const { loading, value, maxTagCount } = this.state;
    const tagCountType = (value && value.length) || 0;
    const keyOption = value.map(item => item.key)
    const filteredOptions = users && users.filter(o => !keyOption.includes(`${o.id}`))
...
export default FilterUsersSelect;