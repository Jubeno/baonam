...
const previewTemplate = `
  <div class="dz-preview dz-file-preview">
    ...
    <div class="dz-error-message" style=top:160px><span data-dz-errormessage></span></div>
  </div>
`
...
class ImageUpload extends React.Component {
  constructor(props) {
    ...
    const dictCancelUploadConfirmation = 'Bạn có chắc chắc muốn hủy tải ảnh không ?'
    const dictRemoveFileConfirmation = "Bạn có chắc chắc muốn xóa ảnh không ?"
  ...
  resetImageEditor = () => {
    ...
    if (iiframe && iiframe.contentWindow && iiframe.contentWindow.canv) {
      ...
      const containerEditor = document.getElementById(`container-transform-${this.editorId}`);
      if (containerEditor)
  ...
  addImages(dropzone, screenShoots) {
    ...
    const images = screenShootss !== '' ? screenShootss.split(',').filter(item => {
      ...
      if (item.indexOf('http') === -1) {
        url = `${publicRuntimeConfig.IMAGE_DAS_SERVER}${publicRuntimeConfig.IMAGE_PROJECT}/${item}`
      }
      else {
        url = item
      }
    ...
    (images || []).forEach((i) => {
      ...
      console.log("dsfdsfs: ", dropzone.files.filter(c =>
        ...
        c.name === i.path && c.status === undefined
      ))
  ...
  preloadImages(dropzone) {
    ...
    const images = screenShootss !== '' ? screenShootss.split(',').map(item => {
      let url = ''
      if (item.indexOf('http') === -1) {
        console.log(`${publicRuntimeConfig.IMAGE_DAS_SERVER}${publicRuntimeConfig.IMAGE_PROJECT}/${item}`)
        url = `${publicRuntimeConfig.IMAGE_DAS_SERVER}${publicRuntimeConfig.IMAGE_PROJECT}/${item}`
      }
      else {
        url = item
  ...
  render() {
    ...
    djsConfig = {
      ...
      transformFile: function (file, done) {
        ...
        const containerEditor = document.getElementById(`container-transform-${editorId}`);
        ...
        if (iiframe) {
          ...
          img.onload = function () {
            ...
            const imgHeight = img.height;
            iiframe.contentWindow.canv.fabric.clear();
            console.log("imgHeight: %o, imgWidth: %o", imgHeight, imgWidth)
            const dontFit = true;
            iiframe.contentWindow.canv.loadMainImage(URL.createObjectURL(file), imgHeight, imgWidth, dontFit);
          }
        } else {
    ...
    return (
      <React.Fragment>
        ...
        <div
          id={`container-transform-${this.editorId}`}
        ...
        >
          <Row type="flex" justify="space-between">
            <Col span={24}>
              <div
                id={`crop-container-transform-${this.editorId}`}
