...
@connect(({ groupCustomers: { dataAll } }) => ({
  groupCustomers: dataAll || []
}))
class Index extends Component {
  ...
  render() {
    const { groupCustomers, key, placeholder } = this.props;
    const { loading, value, maxTagCount } = this.state;
    const tagCountType = (value && value.length) || 0;
    const keyOption = value.map(item => item.key)
    const filteredOptions = groupCustomers && groupCustomers.filter(o => !keyOption.includes(`${o.id}`))
