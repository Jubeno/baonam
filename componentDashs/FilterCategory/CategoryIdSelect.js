...
class Index extends Component {
  ...
  renderTagPlaceHolder = (valueChannel) => (
    ...
    >
      bạn đã chọn {valueChannel && valueChannel.length} chuyên mục
    </a>
  )

  render() {
    const { categories, key, placeholder } = this.props;
    const { loading, value, maxTagCount } = this.state;
    const tagCountType = (value && value.length) || 0;
    const keyOption = value.map(item => item.key)
    const filteredOptions = categories && categories.list && categories.list.filter(o => !keyOption.includes(`${o.id}`))
