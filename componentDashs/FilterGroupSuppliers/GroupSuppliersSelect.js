...
@connect(({ groupSuppliers: { dataAll } }) => ({
  groupSuppliers: dataAll || []
}))
class Index extends Component {
  ...
  renderTagPlaceHolder = (valueChannel) => (
    ...
    >
      bạn đã chọn {valueChannel && valueChannel.length} nhóm nhà cung cấp
    </a>
  )

  render() {
    const { groupSuppliers, key, placeholder } = this.props;
    const { loading, value, maxTagCount } = this.state;
    const tagCountType = (value && value.length) || 0;
    const keyOption = value.map(item => item.key)
    const filteredOptions = groupSuppliers && groupSuppliers.filter(o => !keyOption.includes(`${o.id}`))
