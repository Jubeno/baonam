...
class Index extends React.Component {
  ...
  render() {
    ...
    const dataArticlev1 = dataArticleAll && dataArticleAll.filter((item) => (`${item.categoriesId}` === `${query.id}`))
    const dataArticlev2 = dataArticleAll && dataArticleAll.filter((item) => (`${item.categoriesId}` !== `${query.id}`))
